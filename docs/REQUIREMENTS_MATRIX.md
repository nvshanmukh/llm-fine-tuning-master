...
| 7 | Pin model + tokenizer + revision | PARTIAL | configs, `data/README.md` | revision recorded | model id pinned; **revision SHA not pinned** | add `revision:` to configs |
...
| 13 | Post-training inference quantization vs full precision | IMPLEMENTED_UNVERIFIED / BLOCKED | `quantize.py`, `predict.py` 4/8-bit | load, run same examples, compare | code; **bitsandbytes needs CUDA** | run on CUDA host |
...
| 17 | Meaningful tests + lint/type/format | VERIFIED | `tests/` (105), ruff | counts reported | `pytest` all passed; `ruff check .` clean; mypy see VALIDATION.md | — |
| 17 | No hardcoded personal paths / secrets; `.env.example`, `.gitignore` | VERIFIED | repo-wide | grep clean | `.env.example`, `.gitignore`, `.dockerignore` updated | — |
