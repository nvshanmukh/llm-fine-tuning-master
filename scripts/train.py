...
def main(
    ...
    if ablation:
        _run_ablation(cfg, ablation_config, data_dir)
    else:
        _run_single(cfg, data_dir, experiment_name)
...
def _run_single(cfg, data_dir: str, experiment_name=None):
    """Run a single training experiment."""
    from src.training.trainer import run_training

    train_ds, val_ds = _load_data(data_dir)
...
def _run_ablation(cfg, ablation_config_path: str, data_dir: str):
    ...
    for i, (rank, lr, epochs) in enumerate(experiments, 1):
        ...
        run_cfg = OmegaConf.merge(cfg, OmegaConf.create({
            "peft": {"r": rank},
